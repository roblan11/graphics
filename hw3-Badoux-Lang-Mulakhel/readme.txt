...
Animation:
As mentionned in the pdf document, we simply touch the file `grid/grid_vshader.glsl`. We want to have something that looks like waves moving as a function of time. In order to achieve this, we insert parallel smooth uphills and downhills which moves according to some arbitrary speed. The parameters can easily be defined by the user.
